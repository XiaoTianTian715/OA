...
<table cellspacing="0" cellpadding="0" id="senfe" width="100%" >
		<thead>	<tr>
				<td><select name="selectID" id="selectID" dir="ltr" onchange="selected(this)">
<option value="0">升降排列</option>				
<option value="newsListServletdel?addID=1">增序   </option>
<option value="newsListServletdel?addID=2">降序   </option>
</select></td>
				<th><select name="selectID" id="selectID" dir="ltr" onchange="selected(this)">
<option value="0">新闻标题</option>				
<option value="newsListServletdel?addID=3">增序   </option>
<option value="newsListServletdel?addID=4">降序   </option>
</select></th>
				<th>新闻内容</th>
				<th>发布时间</th>
                 <th ><select name="selectID" id="selectID" dir="ltr" onchange="selected(this)">
<option value="0">发布人</option>				
<option value="newsListServletdel?addID=5">增序   </option>
<option value="newsListServletdel?addID=6">降序   </option>
</select></th>
				<th>新闻类别</th>
				<th><select name="selectID" id="selectID" dir="ltr" onchange="selected(this)">
<option value="0">审核状态</option>				
<option value="newsListServletdel?addID=7">增序   </option>
<option value="newsListServletdel?addID=8">降序   </option>
</select></th>
                 <th><a href="AddNews.jsp">添加  </a></th>
				<th >&nbsp;</th>
			</tr>
			</thead>
			<tbody id="group_one">
			<%ArrayList<News> news= (ArrayList)session.getAttribute("newslist");%>
			<%ClassesDaoImpl clas= (ClassesDaoImpl)session.getAttribute("cladao");%>
			
<%for(int i=0;i<news.size();i++){%>  
			<tr>
				<th ><%=news.get(i).getNewsid() %></th>
				<td style=" "><%=news.get(i).getTitle() %></td>
                ...
                <td style="width: 45px;"><a href="SearchById?newsId=<%=news.get(i).getNewsid()%>">修改</a></td>
                 <td style="width: 45px; "><a href="DoDelServlet?Id=<%=news.get(i).getNewsid()%>">删除</a></td>
			</tr>
			<%}%>
